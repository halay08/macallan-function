...
export default class FirestoreCollection<T extends IEntity> {
   ...
   * @returns
   */
  private _mapDocField(
    doc: IDocumentSnapshot<IDocumentData>,
    data: object = {}
  ) {
    return doc.exists
      ? {
   ...
   * @returns T
   */
  private async _mapDocReference(
    doc: IDocumentSnapshot<IDocumentData>,
    recursive: number = 0
  ): Promise<T> {
    const data = doc.data() || {};
   ...
   */
  async getAll(
    refs: Array<IDocumentReference> = [],
    recursive: number = 0
  ): Promise<T[]> {
    const documentData = await admin.firestore().getAll(...refs);

    return Promise.all(
      documentData.map(doc => this._mapDocReference(doc, recursive))
    );
  }
  ...
  async query(
    ...
    return Promise.all(
      documentData.docs.map(doc => this._mapDocReference(doc, recursive))
    );
  }
